...
function loadAllReportsLive() {
  console.log("[reports] Loading all reports...");
  ...
  reportsUnsubscribe = db.collection('dailyReports')
    .onSnapshot(snapshot => {
      ...
      console.log("[reports] Live reports loaded:", allReports.length, allReports);
      ...
      if (error.code === 'permission-denied') {
        reportsList.innerHTML = `
          <div class="text-center py-12">
            ...
            <h3 class="text-lg font-bold text-gray-800 mb-2">Permission Required</h3>
            <p class="text-gray-600 mb-4">You don't have permission to view team reports.</p>
            <p class="text-sm text-gray-500">Please contact your administrator to grant access to the dailyReports collection in Firestore.</p>
          </div>
        `;
        ...
        reportsList.innerHTML = `
          <div class="text-center py-12">
            ...
            <h3 class="text-lg font-bold text-gray-800 mb-2">Error Loading Reports</h3>
...
function applyReportFilters() {
  ...
  console.log("[reports] Applying filters - Total reports:", allReports.length, "Date filter:", dateFilter);
  ...
  filteredReports = allReports.filter(report => {
    ...
    console.log("[reports] Filter check -", report.userEmail, "Search:", matchesSearch, "Person:", matchesPerson, "Date:", matchesDate);
    
    return matchesSearch && matchesPerson && matchesDate;
  });
  
  console.log("[reports] Filtered reports:", filteredReports.length);
...
function applyReportDateFilter(report, dateFilter) {
  ...
  switch (dateFilter) {
    case 'today':
      // Direct string comparison for today's date
      console.log("[reports] Comparing dates - Report:", report.date, "Today:", todayStr);
    ...
    case 'month':
      ...
      return report.date >= monthAgoStr;
    ...
    case 'custom':
      ...
      if (fromDate && toDate) {
        ...
        return report.date >= fromDate;
      } else if (toDate) {
        return report.date <= toDate;
      }
      return true;
